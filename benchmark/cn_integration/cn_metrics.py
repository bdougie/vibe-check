...
class CNMetricsCollector:
    ...
    def analyze_cn_output(self, stdout: str, stderr: str) -> Dict[str, Any]:
        ...
        for metric_name, pattern_list in patterns.items():
            ...
            for pattern in pattern_list:
                matches = re.finditer(
                    pattern, combined_output, re.IGNORECASE | re.MULTILINE
                )
        ...
        metrics["tool_calls"] = (
            metrics["files_read"] + metrics["files_written"] + metrics["bash_commands"]
        )
    ...
    def analyze_task_completion(self, task_file: str, cn_output: str) -> Dict[str, Any]:
        ...
        for criteria in success_criteria:
            ...
            analysis["criteria_analysis"].append(
                {"criteria": criteria, "met": met, "confidence": 0.5}
            )
            if met:
        ...
        total_items = (
            analysis["total_requirements"] + analysis["total_success_criteria"]
        )
    ...
    def _check_requirement_met(self, requirement: str, cn_output: str) -> bool:
        ...
        key_words = [
            w
            for w in req_words
            if len(w) > 3 and w not in ["the", "and", "that", "this"]
        ]
