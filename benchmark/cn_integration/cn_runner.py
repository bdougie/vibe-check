...
class CNRunner:
    ...
    def __init__(self, working_dir: Optional[Path] = None, verbose: bool = False):
        ...
        if not self._check_cn_available():
            raise CNExecutionError(
                "Continue CLI (cn) is not installed. Run: npm i -g @continuedev/cli"
            )
    ...
    def _check_cn_available(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["cn", "--help"], capture_output=True, text=True, timeout=10
            )
    ...
    def _execute_cn_command(
        ...
        self.logger.debug(
            f"Executing CN command: {' '.join(shlex.quote(arg) for arg in cmd)}"
        )
    ...
    def run_task(
        ...
        try:
            ...
            self.logger.info(
                f"CN task completed: {task_name} - Success: {result['success']} - Time: {execution_time:.2f}s"
            )
