...
class CNBatchIntegration:
    ...
    def run_cn_task(
        self, task_file: str, model_spec: str, timeout: int = 600
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            cn_result = self.cn_runner.run_task(
                task_file=task_file, model_name=model_spec, timeout=timeout
            )
            ...
            logger.info(
                f"CN task completed: {task_name} - Success: {batch_result['success']}"
            )
            return batch_result
    ...
    def _convert_cn_result_to_batch_format(
        self, cn_result: Dict[str, Any], task_name: str
    ) -> Dict[str, Any]:
            ...
            for model_info in models:
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        print(
                            f"✅ {model_name}: PASSED ({result['completion_time']:.1f}s)"
                        )
                    else:
    ...
    def _generate_batch_summary(
        ...
        if successful:
            ...
            efficiency_ranking = sorted(
                successful, key=lambda x: x.get("tool_calls", 999)
            )

            summary["rankings"] = {
                "fastest": speed_ranking[0]["model"] if speed_ranking else None,
                "most_efficient": efficiency_ranking[0]["model"]
                if efficiency_ranking
                else None,
                "most_autonomous": speed_ranking[0][
                    "model"
                ],  # CN is always fully autonomous
            }
            ...
            summary["summary_stats"] = {
                ...
                "total_files_modified": sum(
                    r.get("files_modified", 0) for r in successful
                ),
