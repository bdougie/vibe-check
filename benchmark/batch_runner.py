...
# Added support for automated execution using CN CLI


def run_batch_benchmark_with_cn(
    task_file: str,
    models: Optional[List[str]] = None,
    timeout: int = 1800,
    output_dir: Optional[str] = None,
) -> Dict[str, any]:
    """Run a benchmark task across multiple models using Continue CLI.
    ...
    print("\n🤖 Continue CLI Batch Mode")
    print("=" * 50)
    print("Running fully automated benchmarks using CN CLI")
    print("No human intervention required!")
    print("=" * 50)

    # Initialize CN batch integration
    cn_integration = CNBatchIntegration(verbose=True)

    # Convert model names to model info dictionaries
    model_infos = []
    if models and models != ["all"]:
        for model in models:
            model_infos.append({"name": model})
    else:
        # Use a default set of models for CN testing
        model_infos = [
            {"name": "gpt-3.5-turbo"},
            {"name": "ollama/llama2"},
        ]
        print("ℹ️  Using default models for CN testing")

    # Run the batch with CN
    try:
        result = cn_integration.run_batch_with_cn(
        ...
        print(f"\n❌ CN Batch failed: {e}")
        return {
            "batch_id": f"cn_failed_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            "success": False,
            "error": str(e),
...
# Add new CLI argument parsing for CN mode
if __name__ == "__main__":
    # Check if --cn flag is present
    cn_mode = "--cn" in sys.argv
    if cn_mode:
        sys.argv.remove("--cn")
    ...
    parser.add_argument(
        "--cn",
        action="store_true",
        help="Use Continue CLI for automated execution (no human intervention)",
    )
    ...
    if cn_mode or hasattr(args, "cn") and args.cn:
        # Use Continue CLI batch runner
        runner_result = run_batch_benchmark_with_cn(
            task_file=args.task,
            models=models,
            timeout=args.timeout,
        ...
        # Use original manual batch runner
        runner = BatchRunner(output_dir=args.output)
        runner_result = runner.run_batch_benchmark(
            task_file=args.task, models=models, timeout=args.timeout
        )
