...
from benchmark.task_runner import run_benchmark_task, validate_task_file
...
class BatchRunner:
    ...
    def _detect_ollama_models(self) -> List[str]:
        ...
        try:
            ...
            if self.console:
                self.console.print(f"[yellow]Error detecting Ollama models: {e}[/yellow]")
            return []
    ...
    def _load_configured_models(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            if self.console:
                self.console.print(f"[yellow]Could not load models config: {e}[/yellow]")
            return []
    ...
    def run_batch_benchmark(
        ...
        if not available_models:
            if self.console:
                self.console.print("[red]No models available. Please install Ollama or configure models.[/red]")
            else:
                print("❌ No models available. Please install Ollama or configure models.")
            sys.exit(1)
        ...
        if models and models != ["all"]:
            ...
            for model_spec in models:
                ...
                for available in available_models:
                    if model_spec in available['name'] or model_spec == available['name']:
                        selected_models.append(available)
                        found = True
                        break
                if not found:
                    if self.console:
                        self.console.print(f"[yellow]Warning: Model '{model_spec}' not found[/yellow]")
                    else:
        ...
        if resume_from:
            ...
            models_to_run = [m for m in models_to_run if m['name'] not in completed_models]
        
        # Run benchmarks with progress tracking
        if RICH_AVAILABLE and self.console:
            results = self._run_with_progress(task_file, task_name, models_to_run, timeout)
        else:
    ...
    def _run_with_progress(
        ...
        with Progress(
            ...
            for i, model in enumerate(models):
                progress.update(
                    task,
                    description=f"[cyan]Testing {model['display_name']}..."
                )
    ...
    def _generate_comparison_report(self, results: List[Dict], task_name: str) -> Dict:
        ...
        comparison = {
            ...
            "rankings": {
                ...
                "most_efficient": min(successful, key=lambda x: x['prompts_sent'])['model'] if successful else None,
                "least_intervention": min(successful, key=lambda x: x['human_interventions'])['model'] if successful else None
            },
    ...
    def _calculate_summary_stats(self, results: List[Dict]) -> Dict:
        ...
        return {
            "avg_completion_time": statistics.mean([r['completion_time'] for r in results]),
            "median_completion_time": statistics.median([r['completion_time'] for r in results]),
            "avg_prompts": statistics.mean([r['prompts_sent'] for r in results]),
            "avg_interventions": statistics.mean([r['human_interventions'] for r in results]),
    ...
    def _generate_html_report(self, comparison: Dict):
        ...
        for result in comparison['results']:
            status = '<span class="success">✅ Success</span>' if result['success'] else '<span class="failed">❌ Failed</span>'
            html_content += f"""
                <tr>
                    <td>{result['model']}</td>
                    <td>{status}</td>
    ...
    def _generate_csv_report(self, comparison: Dict):
        ...
        with open(csv_file, 'w', newline='') as f:
            fieldnames = [
                'model', 'task', 'success', 'completion_time',
                'prompts_sent', 'human_interventions', 'files_modified',
    ...
    def _display_summary(self, comparison: Dict):
        ...
        if RICH_AVAILABLE and self.console:
            ...
            if comparison.get('rankings') and comparison['rankings'].get('fastest'):
                ...
                rankings_table.add_row("💬 Most Efficient", comparison['rankings']['most_efficient'])
                rankings_table.add_row("🤖 Most Autonomous", comparison['rankings']['least_intervention'])
            ...
            if comparison.get('rankings') and comparison['rankings'].get('fastest'):
                ...
                print(f"🤖 Most Autonomous: {comparison['rankings']['least_intervention']}")
...
def main():
    ...
    parser.add_argument(
        "--resume",
        help="Resume from a previous batch ID",
        default=None
    )
