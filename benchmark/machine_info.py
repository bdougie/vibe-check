...
class MachineInfoCollector:
                    ...
                    for line in lines:
                        if "Chipset Model:" in line:
                            ...
                            if "apple_silicon" in gpu_info:
                                gpu_info["apple_silicon"]["memory"] = line.split(":")[
                                    1
                                ].strip()
            except Exception:
                pass
    ...
    def print_summary(cls) -> None:
        ...
        print(
            f"System: {info['system']['platform']} {info['system']['platform_release']}"
        )
        ...
        print(
            f"  Cores: {info['cpu']['physical_cores']} physical, {info['cpu']['logical_cores']} logical"
        )

        print(f"\nMemory: {info['memory']['total_gb']} GB total")
        print(
            f"  Available: {info['memory']['available_gb']} GB ({100 - info['memory']['percent_used']:.1f}%)"
        )

        print(f"\nDisk: {info['disk']['total_gb']} GB total")
        print(
            f"  Free: {info['disk']['free_gb']} GB ({100 - info['disk']['percent_used']:.1f}%)"
        )
