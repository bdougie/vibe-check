...
class ModelVerifier:
    ...
    def check_installed_models(self) -> List[str]:
        ...
        if not self.system_info["ollama_running"]:
            self.print_message("Ollama is not running. Starting Ollama check...", "warning")
            return []
    ...
    def get_missing_models(self, priority: Optional[str] = None) -> List[ModelInfo]:
        ...
        for model_name, model_info in RECOMMENDED_MODELS.items():
            ...
            if base_name not in self.installed_models and model_name not in self.installed_models:
    ...
    def calculate_download_requirements(self, models: List[ModelInfo]) -> Dict[str, any]:
        ...
        return {
            ...
            "can_download": self.system_info["disk_free_gb"] > (total_size_bytes / (1024 ** 3) + 5),
    ...
    def suggest_models_for_system(self) -> List[ModelInfo]:
        ...
        for model in suggested:
            if total_size + (model.size_bytes / (1024 ** 3)) < disk_gb - 5:  # Keep 5GB free
                available_models.append(model)
                total_size += model.size_bytes / (1024 ** 3)
        
        return available_models if available_models else [
            m for m in RECOMMENDED_MODELS.values() 
            if m.priority == "essential"
        ][:2]  # At least return 2 essential models
    ...
    def print_model_status(self) -> None:
        ...
        if self.installed_models:
            ...
            for model in self.installed_models:
                ...
                if model in RECOMMENDED_MODELS:
                    ...
                    self.print_message(f"  {status} {model} - {info.description}", "info")
                else:
        ...
        if missing_recommended:
            self.print_message(f"\n⚠️  Missing Recommended Models ({len(missing_recommended)}):", "warning")
    ...
    def suggest_fallbacks(self) -> None:
        ...
        if self.system_info["ram_gb"] < 8:
            ...
            self.print_message("  • Using cloud-based models (OpenAI, Anthropic)", "info")
    ...
    def run_verification(self) -> Dict[str, any]:
        ...
        results["missing_essential"] = [m.name for m in self.get_missing_models("essential")]
        results["missing_recommended"] = [m.name for m in self.get_missing_models("recommended")]
        results["missing_optional"] = [m.name for m in self.get_missing_models("optional")]
        results["suggested"] = [m.name for m in self.suggest_models_for_system()]
        
        # Ready if at least one essential model is installed
        essential_models = [m.name for m in RECOMMENDED_MODELS.values() if m.priority == "essential"]
        results["ready"] = any(model in results["installed"] for model in essential_models)
...
def main():
    ...
    parser.add_argument("--download", "-d", action="store_true", help="Show download commands")
    parser.add_argument("--suggest", "-s", action="store_true", help="Suggest models for your system")
