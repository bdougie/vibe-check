...
class TestValidateModelName:
    ...
    def test_command_injection_attempts(self):
        ...
        for name in malicious_names:
            ...
            assert "Invalid model name format" in str(
                exc_info.value
            ) or "suspicious pattern" in str(exc_info.value)
...
class TestSanitizeErrorMessage:
    ...
    def test_known_error_types(self):
        ...
        test_cases = [
            ...
            (
                ConnectionError("Connection to 192.168.1.1:22 failed"),
                "Connection failed",
            ),
            (
                TimeoutError("Timeout connecting to internal.server:3306"),
                "Operation timed out",
            ),
