...
class SetupWizard:
    ...
    def run(self) -> None:
        ...
        print_info(f"This wizard will guide you through setting up Vibe Check on {self.os_type}")
        print()
    ...
    def check_python_version(self) -> bool:
        ...
        if self.python_version >= min_version:
            ...
            print_error(f"Python {current} is too old. Please install Python 3.8 or higher.")
            print_info("Visit https://www.python.org/downloads/ to download the latest version.")
            return False
    ...
    def check_dependencies(self) -> bool:
        ...
        if shutil.which("uv"):
            ...
            print_info("uv is not installed (optional but recommended for faster installs)")
            if ask_yes_no("Would you like to install uv for faster package management?"):
                if self.install_uv():
                    self.has_uv = True
    ...
    def install_python_dependencies(self) -> bool:
        ...
        if not Path("requirements.txt").exists():
            print_error("requirements.txt not found. Are you in the vibe-check directory?")
            return False
    ...
    def parse_ollama_models(self, output: str) -> None:
        ...
        if self.ollama_models:
            print_success(f"Found {len(self.ollama_models)} Ollama models: {', '.join(self.ollama_models)}")
    ...
    def start_ollama(self) -> bool:
        ...
        if self.os_type == "Darwin":  # macOS
            ...
            print_info("Please start Ollama manually in another terminal: 'ollama serve'")
    ...
    def configure_continue(self) -> None:
        ...
        if ask_yes_no("Would you like help configuring Continue?"):
            ...
            print_info("   https://marketplace.visualstudio.com/items?itemName=Continue.continue")
            ...
            if self.ollama_models:
                ...
                if RICH_AVAILABLE:
                    console.print(Panel(config_snippet, title="Continue Config", language="yaml"))
                else:
                    print("\n" + config_snippet + "\n")
...
"""
    ...
    def print_completion_message(self) -> None:
        ...
        if self.ollama_models:
            ...
            print_info(f"   python benchmark/task_runner.py ollama/{model} benchmark/tasks/easy/fix_typo.md")
...
def main():
    ...
    try:
        ...
        print_error(f"\nUnexpected error: {e}")
        print_info("Please report this issue at: https://github.com/bdougie/vibe-check/issues")
        sys.exit(1)
