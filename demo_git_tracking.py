...
def demo_git_tracking():
    ...
    if metrics.initial_git_state:
        print(
            f"   ✓ Captured initial commit: {metrics.initial_git_state.get('commit', 'unknown')[:8]}"
        )
        print(
            f"   ✓ Has uncommitted changes: {metrics.initial_git_state.get('has_uncommitted_changes', False)}"
        )
    ...
    if metrics.metrics.get("git_diff_details"):
        ...
        for detail in metrics.metrics["git_diff_details"]:
            print(
                f"   • {detail['filename']}: +{detail['lines_added']}/-{detail['lines_removed']}"
            )
    else:
